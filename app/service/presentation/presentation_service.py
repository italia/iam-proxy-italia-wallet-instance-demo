...
class PresentationService(BaseService):
    ...
    def credential_issuer_list(self, url: str) -> None:
        logger.debug(f"Entering credential_issuer_list method. Params [url: {url}]")
        credential_issuer_entity_configuration = self.call_endpoint(url,self.WELL_KNOWN_CREDENTIAL_PATH,self._create_header(self.APPLICATION_JSON_HEADERS),proxies=self.proxy, no_proxy_domains=self.no_proxy_domains)
