...
class ProviderService(BaseService):
    ...
    def __init__(self, app_state: AppState,  proxy, no_proxy_domains):
        logger.debug("Entering method: init for Provider Service")
    ...
    def wallet_provider_ec(self, wallet_provider_list: list[str], url: str) :
        logger.debug(f"Entering method: wallet_provider_ec. Params [url: {url}]")
        self.wallet_provider_url = self.__check_wallet_provider(wallet_provider_list, url)
        entity_configuration_jwt = self.call_endpoint(url,self.WELL_KNOWN_PATH,self._create_header(self.ENTITY_STATEMENT_HEADERS),proxies=self.proxy, no_proxy_domains=self.no_proxy_domains, parse_response= _parse_entity_statement_jwt)
    ...
    def __check_wallet_provider(self, wallet_provider_list: list[str], wallet_provider):
        logger.debug(f"Entering method: check_wallet_provider. Params [wallet_provider_list: {wallet_provider_list}, wallet_provider: {wallet_provider}]")
