...
from app.utils.utils import base64url_decode, base64url_encode, check_required_claims, sanitize_for_logging
...
1       Mappa chiave valore (https://datatracker.ietf.org/doc/html/rfc9360.html#name-x509-cose-header-parameters, https://datatracker.ietf.org/doc/html/rfc8152.html#section-3.1) che presenta:            - (33;<x5chain>): la catena di certificati X.509 la cui chiave del primo elemento della catena è da usare per validare la firma (la public key dell'issuer)
...
def decode_and_verify_issuer_signed(
    issuer_signed_base64_url: str, expected_namespaces: set, expected_version: str, expected_doc_type: str
) -> dict:
    """
...
def _handle_signature(
    ...
    if isinstance(public_key, ec.EllipticCurvePublicKey):
        ...
        if curve_name not in curve_name_map:
            raise ValueError(
                f"La chiave pubblica estratta dal certificato x509 presenta un curva non supportata: {curve_name}"
            )

        p_curve_name = curve_name_map.get(curve_name, curve_name)

        logger.debug(
            f"🔑 La chiave pubblica estratta dal certificato x509 è una chiave ellitica con curva {p_curve_name}"
        )
    else:
        logger.error("❌ La chiave pubblica estratta dal certificato x509 non è una chiave ellitica")
        raise ValueError("La chiave pubblica estratta dal certificato x509 non è una chiave ellitica")

    # 2. Costruzione della struttura firmata (Sig_structure)
    context = "Signature1"
    external_aad = b""
    sig_structure = [context, protected_header_bytes, external_aad, payload_bytes]
...
def _make_json_serializable(obj):
    if isinstance(obj, dict):
        ...
        try:
            ...
            raise ValueError("Errore nella conversione byte to base64")
        return obj_b64
